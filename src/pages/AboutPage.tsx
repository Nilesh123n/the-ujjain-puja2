    ...
    <div className="pt-24 pb-20 animate-in fade-in duration-300">
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-12">
          <span className="inline-block bg-[#F5E6D0] text-[#e09040] font-bold text-xs px-3.5 py-1 rounded-full uppercase mb-2">
            Vedic Scholars
          </span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {TEAM_PANDITS.map((p, idx) => (
            <div
              key={idx}
              className="bg-white border-2 border-[#F5E6D0] rounded-3xl p-6 text-center shadow-md hover:shadow-xl transition-all transform hover:-translate-y-1"
            >
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#f7ae62] to-[#e09040] text-4xl flex items-center justify-center mx-auto mb-4 border-2 border-white shadow-md">
                {p.avatar}
              </div>
              <h3 className="font-cinzel text-base font-bold text-[#5C3A1E]">{p.name}</h3>
              <span className="text-[11px] font-bold text-[#e09040] block mb-1">{p.role}</span>
              <span className="inline-block bg-[#FDF6EC] text-[#8B6F5E] text-[10px] font-bold px-3 py-0.5 rounded-full border border-[#F5E6D0] mb-3">
                {p.experience}
              </span>
              <p className="text-xs text-[#8B6F5E] leading-relaxed mb-4">{p.desc}</p>
              <div className="flex flex-wrap gap-1 justify-center">
                {p.specialties.map((s, i) => (
                  <span key={i} className="text-[9px] bg-amber-50 text-[#5C3A1E] px-2 py-0.5 rounded-md border border-amber-200">
                    {s}
                  </span>
                ))}
              </div>
